...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const imageBlocks = document.querySelectorAll('.portfolio__block-list');
  
  blocks.forEach((e) => {
    const a = e.querySelectorAll('.portfolio__block-list');
    const btn = e.querySelector('.button');
    let i = 1;

    const showBlocks = function () {
      a[i].classList.remove('hidden');
      i++;
      if (i == a.length) {
        btn.removeEventListener('click', showBlocks);
        btn.textContent = 'Свернуть';
        btn.addEventListener('click', hideBlocks);
        i = 1;
      };
    };

    const hideBlocks = function () {
      for (let i = 1; i < a.length; i++) {
        a[i].classList.add('hidden');
      }
      btn.removeEventListener('click', hideBlocks);
      btn.addEventListener('click', showBlocks);
